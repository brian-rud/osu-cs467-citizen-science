<template>
  <main-layout>
    <div class="container mb-4">
      ...
      <VariableDetails :projectDetails="projectDetails" />
      <div class="row mt-4 mb-1">
        <div class="col">
          <h1 style="color:#187bcd">Observations</h1>
          <hr style="height:3px;color:green"/>
        </div>
      </div>
      <!-- Modal -->
      ...
      <div class="container">
        ...
        <button v-if="checkedRowKeys.length > 0" type="button" class="btn btn-danger my-1" data-bs-toggle="modal" data-bs-target="#confirmDeleteObsModal">
            Delete Selected Observation(s)
        </button>
        <n-config-provider :theme-overrides="themeOverrides">
        <n-space vertical :size="10">
          <n-data-table ref="table" :columns="columns" :data="obsTableData" :pagination="pagination" @update:checked-row-keys="handleCheck"/>
        </n-space>
        </n-config-provider>
  ...
  export default {
    ...
    methods: {
      getProjectDetails: function(){
        axios.get(`${process.env.VUE_APP_API_URL}/projects/info/${this.id}`)
          .then((response) => {
      ...
      getResults: function(){
        axios.get(`${process.env.VUE_APP_API_URL}/projects/info/${this.id}/results`)
          .then((response) => {
      ...
      downloadObservations: function(){
        ...
        axios.get(`${process.env.VUE_APP_API_URL}/projects/info/${this.id}/results/download`, {responseType: 'blob'}).then((response) => {
          ...
          link.download = `${this.projectDetails.title}_data.csv`;
          link.click();
        })
      }
    },
